...
Revision ID: 0bbf86d15299
Revises:
Create Date: 2026-04-17 12:54:13.963287
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint(
            "member_id_1", "member_id_2", "relationship_type", name="uq_relationship"
        ),
        sa.CheckConstraint("member_id_1 != member_id_2", name="ck_no_self_relationship"),
    )
    op.create_index(
        "ix_family_relationships_member_id_1", "family_relationships", ["member_id_1"]
    )
    op.create_index(
        "ix_family_relationships_member_id_2", "family_relationships", ["member_id_2"]
    )
    ...
    op.create_index(
        "ix_chat_sessions_anchor_member_id", "chat_sessions", ["anchor_member_id"]
    )
    op.create_index(
        "ix_chat_sessions_created_at_desc", "chat_sessions", ["created_at"], postgresql_ops={"created_at": "DESC"}
    )
    ...
    op.create_index(
        "ix_sync_log_status_attempts", "sync_log", ["status", "attempts"]
    )
    ...
    op.create_index(
        "ix_llm_audit_log_session_id", "llm_audit_log", ["session_id"]
    )
    op.create_index(
        "ix_llm_audit_log_created_at_desc",
        "llm_audit_log",
        ["created_at"],
